...
var transformToolJson = function(type, json) {
    ...
    var transformed = _.clone(json, true);
    ...
    if (type === 'script') {
        ...
        if (angular.isUndefined(transformed.cliAdapter)) {
            transformed.cliAdapter = _.clone(rawTool.cliAdapter, true);
        }
        if (angular.isUndefined(transformed.requirements)) {
            transformed.requirements = _.clone(rawTool.requirements, true);
        }
    }
...
var getTransformSchema = function() {
    ...
    return _.clone(rawTransform, true);
 ...
 *
 * @param {string} type @param {Array} schema
 * @returns {*}
 */
...
var getSchema = function(type, property, toolType, ref) {
    ...
    if (_.isUndefined(property.schema) && _.isUndefined(property.type)) {
         ...
         */
        return ref ? property : _.clone(property, true);
    } else {
        return ref ? (property.schema || property.type) : (_.clone(property.schema || property.type, true));
    }
};
...
var getAdapter = function (property, ref) {
    ...
    return ref ? property.adapter : _.clone(property.adapter, true);
};
